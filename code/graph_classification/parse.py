...
from GPR_pure_trans_linear import *
...
def parse_method(args, dataset, n, c, d, device):
    ...
    elif args.method == 'GPR':
        if args.use_graph:
            # gnn = parse_method(args.backbone, args, args.hidden_channels, d, device)
            gnn = GPRGNN(d, args.hidden_channels, c, alpha=args.gpr_alpha).to(device)
            model = GPR_Transformer(K_transformer=10, init_alpha=args.gpr_alpha, in_channels=d, out_channels=c,
                        hidden_channels=args.hidden_channels, num_layers=1, use_ffn=False, dropout=args.dropout, use_bn=False, graph_weight=0.5, gnn=gnn, use_graph=args.use_graph).to(device)
        else:
            model = GPR_Transformer(K_transformer=10, init_alpha=args.gpr_alpha, in_channels=d, out_channels=c,
                    hidden_channels=args.hidden_channels, num_layers=1, use_ffn=False, dropout=args.dropout, use_bn=False, graph_weight=0.5, use_graph=args.use_graph).to(device)
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--data_dir', type=str, default='/mnt/ai4sci_develop_fast/chaohaoyuan/Nodeformer/data/')
    ...
    parser.add_argument('--gpr_alpha', type=float, default=0.1, help='KNN parameterss.')
