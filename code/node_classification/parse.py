from models import *
from ours import *
from GPR_pure_trans_linear_ab import *
...
def parse_method(method, args, c, d, device):
    if method == 'gcn':
        ...
        if args.use_graph:
            ...
            model = SGFormer(d, args.hidden_channels, c, num_layers=args.num_layers, alpha=args.alpha, dropout=args.dropout, num_heads=args.num_heads,
        ...
        if args.use_graph:
            gnn=parse_method(args.backbone, args, args.hidden_channels, d, device)
            model = GPR_Transformer(K_transformer=args.K_transformer, init_alpha=args.init_alpha, in_channels=d, out_channels=c,
                        hidden_channels=args.hidden_channels, num_layers=args.ours_layers, use_ffn=False, dropout=args.dropout, use_bn=False, graph_weight=args.graph_weight, gnn=gnn, use_graph=args.use_graph).to(device)
        else:
            model = GPR_Transformer(K_transformer=args.K_transformer, init_alpha=args.init_alpha, in_channels=d, out_channels=c,
                    hidden_channels=args.hidden_channels, num_layers=args.ours_layers, use_ffn=False, dropout=args.dropout, use_bn=False, graph_weight=args.graph_weight, use_graph=args.use_graph).to(device)
